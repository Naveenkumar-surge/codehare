...
const Room = () => {
  ...
  useEffect(() => {
    ...
    socket.on("room-messages", (msgs) => {
      ...
      setMessages(trimmed);
      // Set last text message in typing pad
      const lastTextMsg = trimmed.reverse().find((m) => m.type === "text");
      if (lastTextMsg) setTextContent(lastTextMsg.content);
    ...
    socket.on("room-message", (msg) => {
      ...
      if (msg.type === "text") {
        setTextContent(msg.content); // live typing
      }
    });
